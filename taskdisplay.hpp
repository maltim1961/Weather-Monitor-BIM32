...
/**
 * Touch IRQ interrupt handler (T_IRQ pin, FALLING edge)
 * XPT2046 pulls T_IRQ LOW when screen is touched
 */
void IRAM_ATTR touch_irq_handler() { global.touch_irq_pressed = true; }
...
void TaskDisplay1(void *pvParameters) {
  ...
  if (config.display_type(DISPLAY_1) == LCD) {
    /* Initialize display */
    if (config.display_model(DISPLAY_1) == D_NX4832K035 or
        config.display_model(DISPLAY_1) == D_NX4832T035)
      nextion.init();
  ...
  /* Attach touch IRQ interrupt for ILI9341 display */
  if (config.display_type(DISPLAY_1) == LCD &&
      config.display_model(DISPLAY_1) == D_ILI9341) {
    pinMode(TOUCH_IRQ_PIN, INPUT_PULLUP);
    attachInterrupt(TOUCH_IRQ_PIN, touch_irq_handler, FALLING);
  }

  uint32_t touchDebounce = 0; // touch debounce timestamp
  ...
  while (1) {
    if (config.display_type(DISPLAY_1)) {
      ...
      if (millis() - millis_05s >= 500) {
        ...
        if (config.display_type(DISPLAY_1) == NEOPIXEL) {
          ws2812b_1.brightness(get_brightness(DISPLAY_1),
                               global.reduc[DISPLAY_1]);
          ws2812b_1.refresh();
        }
        ...
        /* WS2812b display slow down points blinking frequency if the device
        ...
        if (isOffTime(DISPLAY_1, buttonWasPressed)) {
          if (nextion.isDisplayOn())
            nextion.displayOff();
          if (ili9341.isDisplayOn())
            ili9341.displayOff();
          if (ws2812b_1.isDisplayOn())
            ws2812b_1.displayOff();
      ...
      if (config.display_type(DISPLAY_1) == LCD) {
        ...
        /* ILI9341 touch IRQ handling: tap to toggle backlight only */
        if (config.display_model(DISPLAY_1) == D_ILI9341) {
          if (global.touch_irq_pressed) {
            global.touch_irq_pressed = false;
            if (millis() - touchDebounce > 300) {
              touchDebounce = millis();
              global.disp_autoOff[DISPLAY_1] = millis();
              ili9341.backlightToggle();
            }
          }
        }
      }
    ...
    if (config.display_type(DISPLAY_2)) {
      ...
      if (config.display_type(DISPLAY_2) == D_NEOPIXEL) {
        if (millis() - disp_millis > 500) {
          disp_millis = millis();
          ws2812b_2.brightness(get_brightness(DISPLAY_2),
                               global.reduc[DISPLAY_2]);
          ws2812b_2.refresh();
        }
      }
      ...
      if ((millis() - bright_update) > 1000) {
        ...
        if (isOffTime(DISPLAY_2, buttonWasPressed)) {
          if (ws2812b_2.isDisplayOn())
            ws2812b_2.displayOff();
...
unsigned int get_brightness(unsigned int display_num) {
  ...
  case 0:
    return weather.get_isDay() ? config.display_brightness_day(display_num)
  ...
  case 2: {
    unsigned int morning = config.display_dayTime(display_num, HOUR) * 60 +
                           config.display_dayTime(display_num, MINUTE);
    unsigned int evening = config.display_nightTime(display_num, HOUR) * 60 +
                           config.display_nightTime(display_num, MINUTE);
 ...
 */
bool isOffTime(unsigned int dispNum, bool buttonWasPressed) {
  if (config.display_autoOff(dispNum) > 0 and
      ((millis() - global.disp_autoOff[dispNum]) >
       (config.display_autoOff(dispNum) * 60000))) {
    global.reduc[dispNum] = true;
    if ((millis() - global.disp_autoOff[dispNum]) >
        ((config.display_autoOff(dispNum) * 60000) + 5000))
      return true;
  } else
    global.reduc[dispNum] = false;
  ...
  if (config.display_nightOff(dispNum) && !buttonWasPressed) {
    ...
    if (hour() < config.display_nightOff_from(dispNum))
      timestampFrom -= 86400;
    ...
    if (timestampFrom > timestampTo)
      timestampTo += 86400;
    if (timestampFrom <= now() && now() < timestampTo)
      return true;
  }
