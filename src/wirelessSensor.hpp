...
private:
  ...
  char _sensorType[WSENSORS][20] = {
      "", ""}; // Temperature, Humidity, Pressure sensor type (name)
  ...
  while (Serial2.available()) {
    ...
    if (wsensorStr.lastIndexOf("{") != -1) {
      ...
      if (number >= 0 and number < WSENSORS) {
        ...
        strlcpy(_sensorType[number], root["s"] | "",
                sizeof(_sensorType[number]));
        ...
        _batteryVoltage[number] =
            (float)_adc[number] / (300.0 - config.wsensor_bat_k(number));
        if (_batteryVoltage[number] > 0.0) {
          float umin = 3.75;
          float umax = 3.9;
          if (config.wsensor_bat_type(number) == 0)
            umax = 4.5;
          float stp = (umax - umin) / 4;
          if (_batteryVoltage[number] < (umin + stp))
            _batteryLevel[number] = 1;
          else if (_batteryVoltage[number] < (umin + stp * 2))
            _batteryLevel[number] = 2;
          else if (_batteryVoltage[number] < (umin + stp * 3))
            _batteryLevel[number] = 3;
          else
            _batteryLevel[number] = 4;
          _batteryPercentage[number] =
              (_batteryVoltage[number] - umin) * 100.0 / (umax - umin);
          if (_batteryPercentage[number] > 100.0)
            _batteryPercentage[number] = 100.0;
        }

        Serial.printf("Sensor %d updated", number);
        if (_updated[number] > 0)
          Serial.printf(" at: %d:%02d:%02d\r\n", hour(_updated[number]),
                        minute(_updated[number]), second(_updated[number]));
        else
...
String WirelessSensor::get_sensorType(unsigned int num, unsigned int sensor) {
  if (num >= WSENSORS or sensor > 4)
    return "";
  if (sensor == 0)
    return String(_sensorType[num]);
  else if (sensor >= 1 and sensor <= 4)
    return "DS18B20";
  else
    return "";
}
...
String WirelessSensor::get_energyType() { return "PZEM-004t"; }

String WirelessSensor::get_co2Type() { return "Senseair S8"; }

float WirelessSensor::get_temperature(unsigned int num, unsigned int sensor,
                                      bool corr) {
  if (num >= WSENSORS or sensor > 4)
    return 40400.0;
  return _temperature[num][sensor] +
         (corr ? config.wsensor_temp_corr(num, sensor) : 0.0);
}
