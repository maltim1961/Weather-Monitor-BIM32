...
void Sound::play(unsigned int folder, unsigned int track) {
  ...
  if (folder == 1 and track <= 33) {
    if (String(config.lang()) == "de")
      folder = 2;
    else if (String(config.lang()) == "ru")
      folder = 3;
    else if (String(config.lang()) == "pl")
      folder = 4;
    else if (String(config.lang()) == "ua")
      folder = 5;
    if (track == 0)
      track = 24;
  }
 ...
 * Stop playing
 */
void Sound::stopPlaying(void) { _sendCommand(0x16, 0x00, 0x00); }
...
bool Sound::_hourlyCheck() {
  ...
  if (hour() < config.sound_hour_from())
    timestampFrom -= 86400;
...
void Sound::alarm() {
  if (_mp3_found) {
    for (int i = 0; i < ALARMS; i++) {
      if (config.alarm_state(i) and
          config.alarm_weekday(i, weekday() == 1 ? 6 : weekday() - 2)) {
        if (config.alarm_time(i, 0) == hour() and
            config.alarm_time(i, 1) == minute() and _alarm_rang != minute()) {
          play(2, config.alarm_melodie(i));
          _alarm_rang = minute();
        }
      }
    }
    if (minute() == 0 and second() >= 0 and second() <= 5)
      _alarm_rang = 60;
...
void Sound::_sendCommand(uint8_t command, uint8_t hByte, uint8_t lByte) {
  uint8_t sdata[10] = {0x7E,  0xFF,  0x06, command, 0x00,
                       hByte, lByte, 0x00, 0x00,    0xEF};
