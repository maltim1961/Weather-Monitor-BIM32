 ...
 * Set needToPing
 */
void Network::setNeedToPing() { _needToPing = true; }
  ...
  if (anySSID) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      if (knownNetwork >= 0) {
        ...
        Serial.println(config.network_ssid(knownNetwork));
        _connecting(knownNetwork);
      } else {
        Serial.println("No known networks found");
        Serial.println(
            "trying to connect to network 1 in case it's a hidden network");
        _connecting(0);
      }
    }
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (config.network_type()) {
        ...
        if (ip.fromString(config.network_ip()) and
            gateway.fromString(config.network_gw()) and
            subnet.fromString(config.network_mask()) and
            dns1.fromString(config.network_dns1()) and
            dns2.fromString(config.network_dns2()))
          WiFi.config(ip, gateway, subnet, dns1, dns2);
        else
          Serial.println("Error, switching back to dynamic IP");
      }
...
void Network::runAccessPoint() {
  if (!global.apMode) {
    ...
    Serial.println(config.accessPoint_ssid());
    Serial.print("Password: ");
    Serial.println(config.accessPoint_pass());
