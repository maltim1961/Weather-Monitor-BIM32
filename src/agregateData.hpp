...
float AgregateData::lcdTempIn(float *tempSequence) {
  ...
  case 1:
    ...
    break;  /* temperature from weather forecast */
  case 2: { /* temperature from wireless sensor */
    if (wsensor.dataRelevance(config.display_source_tempIn_wsensNum()))
      temp = wsensor.get_temperature(config.display_source_tempIn_wsensNum(),
    ...
    case 2: { /* wireless sensor */
      if (wsensor.dataRelevance(config.display_source_sequence_wsenstemp(i, 0)))
        tempSequence[i] = wsensor.get_temperature(
            ...
            config.display_source_sequence_wsenstemp(i, 1), CORRECTED);
    }; break;
    case 3: { /* thingspeak */
      if (thingspeak.dataRelevance())
        tempSequence[i] =
            thingspeak.get_field(config.display_source_sequence_thngtemp(i));
    }; break;
    case 4:
...
float AgregateData::lcdTempOut() {
  ...
  case 2: { /* temperature from wireless sensor */
    if (wsensor.dataRelevance(config.display_source_tempOut_wsensNum()))
      temp = wsensor.get_temperature(config.display_source_tempOut_wsensNum(),
...
float AgregateData::lcdHumIn(float *humSequence) {
  ...
  case 1:
    ...
    break;  /* humudity from weather forecast */
    ...
    case 2: { /* wireless sensor */
      if (wsensor.dataRelevance(config.display_source_sequence_wsenshum(i)))
        humSequence[i] = wsensor.get_humidity(
            config.display_source_sequence_wsenshum(i), CORRECTED);
    }; break;
    case 3: { /* thingspeak */
      if (thingspeak.dataRelevance())
        humSequence[i] =
            thingspeak.get_field(config.display_source_sequence_thnghum(i));
    }; break;
    case 4:
...
float AgregateData::lcdPresOut() {
  ...
  case 1:
    ...
    break;  // pressure outside from weather forecast
  ...
  case 4:
    ...
    break; // pressure outside from BME280
  case 5:
    pres = sensors.get_bmp180_pres(CORRECTED);
    break; // pressure outside from BMP180
  case 6:
    pres = sensors.get_bme680_pres(CORRECTED);
    break; // pressure outside from BME680
  }
  return pres;
}
...
String AgregateData::lcdVoltage() {
  ...
  case 1:
    value = _lcdVoltageWsensor();
    break; /* from wireless sensor */
  case 2:
    value = _lcdVoltageThingspeak();
    break;  /* from thingspeak */
  case 3: { // iaq from BME680
    float iaq = sensors.get_bme680_iaq(CORRECTED);
    if (validate.iaq(iaq))
      value = "IAQ " + String((int)round(iaq));
    else
      value = "";
  }; break;
  default:;
...
String AgregateData::_lcdVoltageWsensor() {
  ...
  if (wsensor.dataRelevance(config.display_source_volt_wsensNum())) {
    ...
    case 0: { /* battery voltage */
      float volt =
          wsensor.get_batteryVoltage(config.display_source_volt_wsensNum());
      if (validate.batVolt(volt))
        value = String(round(volt * 100) / 100) + lang.v();
    }; break;
    case 1: { /* battery percentage */
      float prc =
          wsensor.get_batteryPercentage(config.display_source_volt_wsensNum());
      if (validate.batPercent(prc))
        value = String(prc, 0) + "%";
    }; break;
    case 2: { /* PZEM-004t voltage */
      float hiVolt =
          wsensor.get_voltage(config.display_source_volt_wsensNum(), CORRECTED);
      if (validate.volt(hiVolt))
        value = String(round(hiVolt * 100) / 100) + lang.v();
    }; break;
    case 3: { /* SenseAir S8 CO2 level */
      float co2 =
          wsensor.get_co2(config.display_source_volt_wsensNum(), CORRECTED);
      if (validate.co2(co2))
        value = String((int)round(co2)) + "ppm";
    }; break;
    default:;
      break;
    }
...
int AgregateData::lcdBatteryLevel() {
  ...
  if (config.display_source_bat_sens() ==
      1) { // battery symbol from wireless sensor
  ...
  } else if (config.display_source_bat_sens() ==
             2) { // battery symbol from thingspeak
    if (thingspeak.dataRelevance()) {
...
float AgregateData::comfortTemp() {
  ...
  case 2:  // temperature from wireless sensor
    if (wsensor.dataRelevance(config.comfort_temp_wsensNum()))
      temp = wsensor.get_temperature(config.comfort_temp_wsensNum(),
                                     config.comfort_temp_sens(), CORRECTED);
    break;
  case 3: // temperature from thingspeak
    if (thingspeak.dataRelevance())
...
float AgregateData::comfortHum() {
  ...
  case 1:
    hum = weather.get_currentHum();
    break; // humidity from weather forecast
  case 2:  // humidity from wireless sensor
