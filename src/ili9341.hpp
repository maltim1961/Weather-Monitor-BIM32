...
#define PM_GOOD_COLOR 0x07E0        // green  (PM2.5 < 12, PM10 < 54)
#define PM_MODERATE_COLOR 0xFFE0    // yellow (PM2.5 < 35, PM10 < 154)
#define PM_UNHEALTHY_S_COLOR 0xFD20 // orange (PM2.5 < 55, PM10 < 254)
#define PM_UNHEALTHY_COLOR 0xF800   // red    (PM2.5 >= 55, PM10 >= 254)
...
private:
  ...
  /* Touch variables */
  uint32_t _touchDebounce = 0;                   // debounce timestamp
  static const uint32_t TOUCH_DEBOUNCE_MS = 300; // min interval between taps
  bool _backlightState = true;                   // backlight on/off state

  void _sequenceSlotSkip();
  void _sequenceSlotNext();
  void _showImg(uint16_t x, uint16_t y, String img);
  void _printText(uint16_t x, uint16_t y, uint16_t width, uint16_t height,
                  String text, uint8_t font, uint8_t align, uint16_t color);
  void _showTemperature(int temp, uint16_t x, uint16_t y, uint8_t font,
                        uint16_t color);
  ...
  /* PM sensor tracking */
  float _prevPM25 = -99.0;
  float _prevPM10 = -99.0;

  uint16_t _pmColor(float pm25val, bool isPM10, float pm10val);
};
...
void ILI9341::init(void) {
  ...
  /* Touch calibration for MSP2402 (ILI9341 + XPT2046) */
  /* Calibration data: {x_min, x_max, y_min, y_max, rotation_swap} */
  /* If touch is inaccurate, run TFT_eSPI calibration example and replace these
   * values */
  uint16_t calData[5] = {350, 3600, 250, 3550, 3};
 ...
 */
void ILI9341::displayToggle() { _power = !_power; }

/**
 * Turns on the display
 */
void ILI9341::displayOn() { _power = true; }

/*
 * Turns off the display
 */
void ILI9341::displayOff() { _power = false; }

/**
 * Returns true if display is on
 */
bool ILI9341::isDisplayOn() { return _power; }
...
void ILI9341::_printText(uint16_t x, uint16_t y, uint16_t width,
                         uint16_t height, String text, uint8_t font,
                         uint8_t align, uint16_t color) {
  if (_prevFont != font) {
    if (font == FONT1)
      tft.loadFont(Ubuntu_14);
    else if (font == FONT2)
      tft.loadFont(Ubuntu_21);
    else if (font == FONT3)
      tft.loadFont(Ubuntu_29);
    _prevFont = font;
  }
 ...
 * Display temperature
 */
void ILI9341::_showTemperature(int temp, uint16_t x, uint16_t y, uint8_t font,
                               uint16_t color) {
  String buf = validate.temp(temp) ? (String(temp) + "°C") : "--";
  _printText(x, y, font == FONT3 ? 70 : 56, font == FONT3 ? 26 : 20, buf, font,
             font == FONT3 ? CENTER : RIGHT, color);
}
...
void ILI9341::_drawSkeleton() {
  ...
  /* PM2.5 / PM10 labels in the upper-left area (replacing big clock) */
  _printText(2, 5, 65, 20, "PM2.5:", FONT2, LEFT, TEXT_COLOR);
  _printText(2, 45, 65, 20, "PM10:", FONT2, LEFT, TEXT_COLOR);
}

void ILI9341::_showTime() {
  /* Time display removed — replaced by PM2.5 / PM10 */
}

void ILI9341::_clockPoints() {
  /* Clock points removed — replaced by PM2.5 / PM10 */
}

/**
 * Get color for PM value based on AQI thresholds
 */
uint16_t ILI9341::_pmColor(float pm25val, bool isPM10, float pm10val) {
  float v = isPM10 ? pm10val : pm25val;
  if (isPM10) {
    if (v < 55.0)
      return PM_GOOD_COLOR;
    if (v < 155.0)
      return PM_MODERATE_COLOR;
    if (v < 255.0)
      return PM_UNHEALTHY_S_COLOR;
    return PM_UNHEALTHY_COLOR;
  } else {
    if (v < 13.0)
      return PM_GOOD_COLOR;
    if (v < 36.0)
...
/**
 * Display PM2.5 and PM10 values in the upper-left corner
 * (replaces the big clock digits)
 * Uses FONT3 (Ubuntu_29) — same font as indoor temperature
 */
void ILI9341::_showPM() {
  float pm25 = global.sds011_pm25;
  float pm10 = global.sds011_pm10;

  if (_prevPM25 != pm25) {
    String buf = (pm25 >= 0) ? String(pm25, 1) : "--";
    uint16_t col = (pm25 >= 0) ? _pmColor(pm25, false, 0) : TEXT_COLOR;
    _printText(67, 2, 75, 26, buf, FONT3, RIGHT, col);
    _prevPM25 = pm25;
  }

  if (_prevPM10 != pm10) {
    String buf = (pm10 >= 0) ? String(pm10, 1) : "--";
    uint16_t col = (pm10 >= 0) ? _pmColor(0, true, pm10) : TEXT_COLOR;
    _printText(67, 42, 75, 26, buf, FONT3, RIGHT, col);
    _prevPM10 = pm10;
  }
}
...
void ILI9341::_showWeekday() {
  if (_prevTWeekday != _tWeekday) {
    _printText(146, 6, 40, 20, lang.weekdayShortName(_tWeekday), FONT2, LEFT,
               CLOCK_COLOR);
  }
}
...
void ILI9341::_showWeekdays() {
  if (_prevTWeekday != _tWeekday) {
    ...
    _printText(33, 168, 40, 16, lang.weekdayShortName(wd), FONT1, CENTER,
               TEXT_COLOR);
    if (++wd > 7)
      wd = 1;
    _printText(139, 168, 40, 16, lang.weekdayShortName(wd), FONT1, CENTER,
               TEXT_COLOR);
    if (++wd > 7)
      wd = 1;
    _printText(245, 168, 40, 16, lang.weekdayShortName(wd), FONT1, CENTER,
               TEXT_COLOR);
...
void ILI9341::_showAntenna() {
  if (_prevRssi != _rssi or _prevIsApMode != _isApMode) {
    ...
    if (_isApMode)
      ...
      if (_rssi > -51)
        ant = "ant_4";
      if (_rssi < -50 && _rssi > -76)
        ant = "ant_3";
      if (_rssi < -75 && _rssi > -96)
        ant = "ant_2";
      if (_rssi < -95)
        ant = "ant_1";
      if (_rssi >= 0)
...
void ILI9341::_sequenceSlotNext() {
  if (millis() - _sequenceMillis >
      config.display_source_sequence_dur() * 1000) {
...
void ILI9341::_showTemperatureInside() {
  if (config.display_source_tempIn_sens() == 4)
    _tempIn = _tempSequence[_sequenceSlot];
...
void ILI9341::_showHumidityInside() {
  if (config.display_source_humIn_sens() == 4)
    _humIn = _humSequence[_sequenceSlot];
...
void ILI9341::_showComfort() {
  if (config.display_source_descr() == 2)
    _comfort = _nameSequence[_sequenceSlot];
...
void ILI9341::_showBatteryLevel() {
  if (_prevBatLevel != _batLevel) {
    if (validate.batLvl(_batLevel)) {
      ...
      tft.fillRect(258, 2, 32, 21, BG_COLOR);
...
void ILI9341::_showWeatherIcon() {
  if (_prevCurrIcon != _currIcon or _prevIsDay != _isDay) {
    ...
    switch (_currIcon) {
    case 1:
      path += _isDay ? "01_d" : "01_n";
      break;
    case 2:
      path += _isDay ? "02_d" : "02_n";
      break;
    case 3:
      path += "04";
      break;
...
void ILI9341::_showDescription() {
  if (_prevDescription != _description) {
    ...
    tft.unloadFont();
    _printText(0, 84, 319, 20, _description, w > 316 ? FONT1 : FONT2, CENTER,
               TEXT_COLOR);
...
void ILI9341::_showPressure() {
  if (_prevPresOut != _presOut) {
    String buf = validate.pres(_presOut)
...
void ILI9341::_showWindSpeed() {
  if (_prevWindSpd != _windSpd) {
    String wnd = validate.wind(_windSpd)
...
void ILI9341::_showUpdTime() {
  if (_prevWeatherUpdated != _weatherUpdated) {
    ...
    char buf[32] = "";
    sprintf(buf, "%02d.%02d.%d %02d:%02d:%02d", day(t), month(t), year(t),
...
void ILI9341::_showForecastTemps() {
  for (uint8_t i = 0; i < 3; i++) {
    if (_prevDTemps[i] != _dTemps[i]) {
      _showTemperature(int(round(_dTemps[i])), i * 106 + 49, 183, FONT2,
                       TEMPERATURE_COLOR);
      _prevDTemps[i] = _dTemps[i];
    }

    if (_prevNTemps[i] != _nTemps[i]) {
      _showTemperature(int(round(_nTemps[i])), i * 106 + 49, 203, FONT2,
                       TEMP_MIN_COLOR);
...
void ILI9341::_showForecastWinds() {
  for (uint8_t i = 0; i < 3; i++) {
    if (_prevWinds[i] != _winds[i]) {
      String wnd = validate.wind(_winds[i])
 ...
 * Returns true if a new tap is detected
 * x, y contain calibrated screen coordinates
 */
bool ILI9341::touchRead(uint16_t &x, uint16_t &y) {
  if (millis() - _touchDebounce < TOUCH_DEBOUNCE_MS)
    return false;
  bool pressed = tft.getTouch(&x, &y);
  if (pressed) {
    _touchDebounce = millis();
    return true;
  }
  return false;
}
...
/**
 * Simple touch check with debounce (no coordinates)
 * Returns true if a new tap is detected
 */
bool ILI9341::isTouched() {
  uint16_t x, y;
  return touchRead(x, y);
}

/**
 * Turn backlight on (without affecting display rendering)
 */
void ILI9341::backlightOn() {
  _backlightState = true;
  brightness(_prevBright);
}

/**
 * Turn backlight off (without affecting display rendering)
 */
void ILI9341::backlightOff() {
  _backlightState = false;
  analogWrite(TFT_BACKLIGHT, 0);
}

/**
 * Toggle backlight on/off (without affecting display rendering)
 */
void ILI9341::backlightToggle() {
  if (_backlightState)
    backlightOff();
  else
    backlightOn();
...
/**
 * Returns true if backlight is on
 */
bool ILI9341::isBacklightOn() { return _backlightState; }