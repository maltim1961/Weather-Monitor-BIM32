...
String Weather::_openMeteoCode2Description(uint8_t code) {
  switch (code) {
  case 0:
    return lang.clearSky();
  case 1:
    return lang.mainlyClear();
  case 2:
    return lang.partlyCloudy();
  case 3:
    return lang.overcast();
  case 45:
    return lang.fog();
  case 48:
    return lang.deposRimeFog();
  case 51:
  case 53:
  case 55:
    return lang.drizzle();
  ...
  case 66:
  case 67:
    return lang.freezingRain();
  case 71:
  case 73:
  case 75:
    return lang.snowFall();
  case 77:
    return lang.snowGrains();
  case 80:
  case 81:
  case 82:
    return lang.rainShowers();
  case 85:
  case 86:
    return lang.snowShowers();
  case 95:
    return lang.thunderstorm();
  case 96:
  case 99:
    return lang.thunderstormWithHail();
  default:
    return "---";
  }
}
 ...
 */
void Weather::update() {
  Serial.printf("[Weather] Free heap before update: %u bytes\r\n",
                ESP.getFreeHeap());
  String url = "";
  if (config.weather_provider() == OPENWEATHERMAP) {
    ...
    url = "http://api.weatherbit.io/v2.0/current?key=" +
          config.weather_appid(WEATHERBIT);
    if (config.weather_citysearch() == 0) {
      if (config.weather_city() == "") {
        Serial.println("No Cityname");
        return;
    ...
    if (config.weather_citysearch() == 2) {
      ...
      url += "&current=temperature_2m,relative_humidity_2m,is_day,weather_code,"
  ...
  if (httpCode == HTTP_CODE_OK) {
    WiFiClient *stream = clientWeather.getStreamPtr();
    JsonDocument weather;
    DeserializationError errorWeather = deserializeJson(weather, *stream);
    if (errorWeather) {
      Serial.print("Current weather deserialization error: ");
    ...
    if (config.weather_provider() == WEATHERBIT) {
      ...
      _currentIcon =
          _weatherbitIcon(weather["data"][0]["weather"]["code"].as<int>() | 0);
    }

    if (config.weather_provider() == OPEN_METEO) {
      _description =
          _openMeteoCode2Description(weather["current"]["weather_code"] | 0);
  ...
  Serial.printf("[Weather] Free heap after current weather: %u bytes\r\n",
                ESP.getFreeHeap());
  vTaskDelay(500 / portTICK_PERIOD_MS); // Give heap time to recover

  if (config.weather_provider() == WEATHERBIT)
    _updateWeatherbitDaily();
  if (config.weather_provider() == OPENWEATHERMAP)
    _updateOpenweathermapHourly();
  if (config.weather_provider() == OPEN_METEO) {
...
void Weather::_updateWeatherbitDaily(void) {
  ...
  if (httpCode == HTTP_CODE_OK) {
    WiFiClient *stream = clientDaily.getStreamPtr();
    JsonDocument forecast;
    DeserializationError errorForecast = deserializeJson(forecast, *stream);
    if (errorForecast) {
      Serial.print("Weatherbit.io: daily forecast deserialization error: ");
      Serial.println(errorForecast.c_str());
      clientDaily.end();
      return;
    }
    for (unsigned int i = 0; i < DAYS; i++) {
      ...
      _dailyIcon[i] =
          _weatherbitIcon(forecast["data"][i]["weather"]["code"].as<int>() | 0);
    }
    _dailyUpdated = now();
    Serial.print("Weatherbit.io: daily forecast updated successfully at: ");
    Serial.printf("%02d:%02d:%02d\r\n", hour(), minute(), second());
  } else
    Serial.println("Weatherbit.io: daily forecast update error");
  clientDaily.end();
}
...
void Weather::_updateOpenweathermapHourly(void) {
  ...
  Serial.printf("[Weather] Free heap before hourly forecast: %u bytes\r\n",
                ESP.getFreeHeap());

  // OW_forecast is ~4KB+, check if we have enough heap
  if (ESP.getFreeHeap() < 20000) {
    Serial.println("[Weather] Not enough heap for hourly forecast, skipping");
    return;
  }
  ...
  if (_lat.length() and _lon.length()) {
    ...
    forecast = new (std::nothrow) OW_forecast;
    if (!forecast) {
      Serial.println("[Weather] Failed to allocate OW_forecast");
      return;
    }
    bool parsed = ow.getForecast(forecast, config.weather_appid(OPENWEATHERMAP),
                                 _lat, _lon, "metric", "en", false);
    ...
    Serial.println("City undefined");

  Serial.printf("[Weather] Free heap after hourly forecast: %u bytes\r\n",
                ESP.getFreeHeap());
}
...
void Weather::_updateOpenMeteoDaily() {
  ...
  url += "?latitude=" + config.weather_lat();
  url += "&longitude=" + config.weather_lon();
  url += "&daily=weather_code,temperature_2m_max,temperature_2m_min,wind_speed_"
         "10m_max";
  url +=
      "&wind_speed_unit=ms&timeformat=unixtime&timezone=auto&forecast_days=4";
  HTTPClient clientDaily;
  // Serial.println(url);
  clientDaily.begin(url);
  int httpCode = clientDaily.GET();
  if (httpCode == HTTP_CODE_OK) {
    ...
    DeserializationError errorForecast = deserializeJson(forecast, *stream);
    if (errorForecast) {
      Serial.print("Open-meteo.com: daily forecast deserialization error: ");
      Serial.println(errorForecast.c_str());
      clientDaily.end();
      return;
    }
    ...
    Serial.println("Open-meteo.com: daily forecast update error");
  clientDaily.end();
}
...
void Weather::_updateOpenMeteoHourly() {
  ...
  url +=
      "&wind_speed_unit=ms&timeformat=unixtime&timezone=auto&forecast_days=6";
  HTTPClient clientHourly;
  // Serial.println(url);
  clientHourly.begin(url);
  int httpCode = clientHourly.GET();
  if (httpCode == HTTP_CODE_OK) {
    WiFiClient *stream = clientHourly.getStreamPtr();
    JsonDocument forecast;
    DeserializationError errorForecast = deserializeJson(forecast, *stream);
    if (errorForecast) {
      Serial.print("Open-meteo.com: hourly forecast deserialization error: ");
      Serial.println(errorForecast.c_str());
      clientHourly.end();
      return;
    }
    ...
    for (unsigned int i = 0; i < 144; i++) {
      ...
      if (hr == 0 or hr == 3 or hr == 6 or hr == 9 or hr == 12 or hr == 15 or
          hr == 18 or hr == 21) {
        if ((time + utc) >= now()) {
          ...
          _hourlyPrec[n] =
              forecast["hourly"]["precipitation_probability"][i] | 0.0;
          _hourlyIcon[n] =
              _openMeteoIcon(forecast["hourly"]["weather_code"][i] | 0);
          if (n < 39)
            n++;
          else
            break;
    ...
    Serial.println("Open-meteo.com: hourly forecast update error");
  clientHourly.end();
}
  ...
  for (unsigned int i = 0; i < DAYS * 8; i++) {
    ...
    for (unsigned int k = 0; k < DAYS; k++) {
      if (weekday(_hourlyDate[i]) == wd) {
        if (_dailyDayTemp[k] < _hourlyTemp[i])
          _dailyDayTemp[k] = _hourlyTemp[i];
        if (_dailyNightTemp[k] > _hourlyTemp[i])
          _dailyNightTemp[k] = _hourlyTemp[i];
        if (_dailyIcon[k] < _hourlyIcon[i])
          _dailyIcon[k] = _hourlyIcon[i];
        if (_dailyWindSpeed[k] < _hourlyWindSpeed[i])
          _dailyWindSpeed[k] = _hourlyWindSpeed[i];
      }
      if (++wd > 7)
        wd = 1;
    }
 ...
 * Getters
 */
float Weather::get_currentTemp() { return _currentTemp; }

float Weather::get_currentHum() { return _currentHum; }

float Weather::get_currentPres() { return _currentPres; }

float Weather::get_currentWindSpeed() { return _currentWindSpeed; }

int Weather::get_currentWindDir() { return _currentWindDir; }

unsigned int Weather::get_currentIcon() { return _convertIcon(_currentIcon); }

bool Weather::get_isDay() { return _is_day; }

String Weather::get_description() { return _description; }

time_t Weather::get_currentUpdated() { return _currentUpdated; }
