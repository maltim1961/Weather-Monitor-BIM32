...
public:
  ...
  void comfortDevices(bool heater, bool cooler, bool humidifier,
                      bool dehumidifier, bool purifier);
...
void Sensors::init(void) {
  ...
  Serial.printf("%s %s%s\r\n", "DS18B20:  ", _ds18b20_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "BME280:   ", _bme280_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "BMP180:   ", _bmp180_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "SHT21:    ", _sht21_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "DHT22:    ", _dht22_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "MAX44009: ", _max44009_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "BH1750:   ", _bh1750_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "BME680:   ", _bme680_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "PCF8574:  ", _pcf8574_det ? "" : "NOT ",
                "Detected");
  Serial.printf("%s %s%s\r\n", "DS3231:   ", _ds3231_det ? "" : "NOT ",
                "Detected");
}
...
uint8_t Sensors::get_bme680_iaq_accuracy() { return _bme680_iaq_accuracy; }
...
void Sensors::_BME280Init(void) {
  ...
  if (!_bme280_det)
    _bme280_det = bme.begin(0x77, &Wire);
}
...
#include "config/generic_33v_3s_4d/bsec_iaq.txt"
  ...
  if (_bme680_det) {
    ...
    bsec_virtual_sensor_t sensorList[4] = {
        BSEC_OUTPUT_IAQ, BSEC_OUTPUT_RAW_PRESSURE,
    ...
    while (file.available()) {
      ...
      if (!errorState) {
        ...
        if (_bme680_stateCounter > 0) {
          ...
          Serial.println("state file is empty");
      } else
        Serial.println(" BME680 state file corrupted");
    }
    file.close();
  } else
    Serial.println(" No BME680 file found");
}
...
#define STATE_SAVE_PERIOD UINT32_C(6 * 60 * 60) /* 6 hours - 4 times a day */
  ...
  if ((now() - _bme680_stateTimestamp >= STATE_SAVE_PERIOD) and
    ...
    if (LittleFS.exists("/bme680.json")) {
      ...
      char datetime[24];
      snprintf(datetime, sizeof(datetime), "%02d.%02d.%d %02d:%02d:%02d", day(),
      ...
      if (file) {
        ...
        Serial.println("Failed to save BME680 state file");
    } else
      Serial.println("File /bme680.json does not extist");
  }
}
...
void Sensors::get_ds3231_timeDate() {
  if (_ds3231_det) {
    rtc.read();
    setTime(rtc.hours(), rtc.minutes(), rtc.seconds(), rtc.day(), rtc.month(),
            2000 + rtc.year());
  }
}
 ...
 */
void Sensors::comfortDevices(bool heater, bool cooler, bool humidifier,
                             bool dehumidifier, bool purifier) {
  if (_pcf8574_det) {
