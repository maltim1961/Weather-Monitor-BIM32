...
void TaskSensors(void *pvParameters) {
  ...
  while (1) {
    ...
    if (digitalRead(SETTINGS_BUTTON_PIN) == 0)
      network.runAccessPoint();
    ...
    if (!global.apMode) {
      ...
      if (config.clock_ntp_period() > 0) {
        if ((millis() - ntp_update) > config.clock_ntp_period() * 60000 or
      ...
      if (config.thingspeakReceive_period() > 0) {
        if ((millis() - thingspeakReceive) >
                (config.thingspeakReceive_period() * 60000) or
            thingspeakReceive == 0) {
      ...
      if (config.thingspeakSend_period() > 0) {
        if ((millis() - thingspeakSend) >
                (config.thingspeakSend_period() * 60000) or
            thingspeakSend == 0) {
      ...
      if (config.narodmonSend_period() > 0) {
        if ((millis() - narodmonSend) >
                (config.narodmonSend_period() * 60000) or
            narodmonSend == 0) {
          narodmonSend = millis();
          Serial.println(SEPARATOR);
    ...
    /* Stop alarm sound if the alarm button was pressed (moved from ISR) */
 ...
 */
void display1_toggle() { global.display_but_pressed[0] = true; }

/**
 * Interrupt from display 2 button
 */
void display2_toggle() { global.display_but_pressed[1] = true; }

/**
 * Interrupt from alarm button
 */
void alarm_button() { global.alarm_but_pressed = true; }

/**
 * Interrupt from mp3 player busy pin
 */
void mp3_busy() { global.mp3_busy = false; }
...
void get_time(void) {
  if (global.net_connected) {
    ...
    if (tmstruct.tm_year >= 121) {
      setTime(tmstruct.tm_hour, tmstruct.tm_min, tmstruct.tm_sec,
              tmstruct.tm_mday, tmstruct.tm_mon + 1, tmstruct.tm_year + 1900);
      unsigned int summertime =
